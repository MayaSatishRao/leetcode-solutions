...
class Solution {
    ...
    /*static int count = 0;
    static int dfs(ArrayList<ArrayList<int[]>> edge, int p, boolean[] visit, int d, int dest, int minCd, int minSd){
         count++;
         visit[p] = true;
         //if(p==6 || p==4)
         System.out.println(p+" "+d+" "+minCd+" "+minSd);
         if(p==dest){
             visit[p]=false;
             int newD = d;
             
             if(minCd!=Integer.MAX_VALUE)
              newD = d-minSd+minCd;
         ...
         for(int i=0;i<edge.get(p).size();i++){
                int next = edge.get(p).get(i)[0], cd = edge.get(p).get(i)[2];
                int sd = edge.get(p).get(i)[1];
                //System.out.println(next+" "+p);
                int tempCd = minCd, tempSd = minSd;
                int curr=0;
                if(!visit[next]){
                    if(minCd> cd && cd<=sd){
                    tempCd=cd;
                    ...
                    curr = dfs(edge,next,visit,d+sd,dest,tempCd,tempSd);
