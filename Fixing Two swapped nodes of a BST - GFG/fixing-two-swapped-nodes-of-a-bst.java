...
class Solution {
    ...
    public void changeVal(Node root,ArrayList<Integer> arr, int[] index){
        if(root!=null){
            changeVal(root.left,arr,index);
            root.data = arr.get(index[0]);
            index[0]++;
            changeVal(root.right,arr,index);
        }
    }
    public Node correctBST(Node root) {
        ...
        if(third==-1){
            ...
            int temp = arr.get(first);
            arr.set(first,arr.get(third));
            arr.set(third,temp);
        }
        
        int[] index = new int[1];
        index[0] = 0;
        changeVal(root,arr,index);
        return root;
    }
}
